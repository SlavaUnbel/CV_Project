...
export const generatePortfolioData = (): IPortfolio[] => {
  ...
  return Array.from({ length: portfolioItems.length }, (_, idx) => idx).map(
    (index) => {
      const paths = portfolioItems.map(
        (item) => `/${item.title.replaceAll(" ", "-").toLowerCase()}`
      );
...
export const generateExpandingCardsData = (): IExpandingCards[] => {
  const titles = ["Viñales", "Oak Alley", "Austria", "Big Sur", "Deutschland"];
  const images = ["viñales", "oak-alley", "austria", "big-sur", "deutschland"];

  return Array.from({ length: titles.length }, (_, idx) => idx).map((index) => {
    return {
      _id: uuid(),
      title: titles[index],
...
export const generateFaqCollapseData = (): IFaqCollapse[] => {
  const faqs = {
    titles: [
  ...
  return Array.from({ length: faqs.count }, (_, idx) => idx).map((index) => {
    return {
      _id: uuid(),
      title: faqs.titles[index],
      answer: faqs.answers[index],
    };
  });
};
...
export const generateTestimonialsSwitcherData = (): ITestimonialsSwitcher[] => {
  const testimonials = [
  ...
  return Array.from({ length: testimonials.length }, (_, idx) => idx).map(
    (index) => {
      return {
        _id: uuid(),
        testimonial: testimonials[index],
        avatar: `${testimonialsSwitcherSrc}/${usernames[index]
          .split(" ")
          .join("-")
          .toLowerCase()}.jpg`,
